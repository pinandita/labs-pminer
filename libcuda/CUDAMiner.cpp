...
// prepare for next algo "CTXC" soon....
Miner* CUMinerFactory::GetCUMinerIntance(unsigned _index, DeviceDescriptor& _device) {
        Miner* miner = nullptr;
        if (_device.cuAlgo == "ethash")
        {
            miner = new CUDAMiner(_index, _device);
        } else if (_device.cuAlgo == "etchash")
        {
            miner = new CUDAMiner(_index, _device);
        } else { // next algo ???
            cnote << "Unrecognized Pow Type";
        }
        return miner;
}
...
#define HostToDevice(dst, src, siz, stream) CUDA_CALL(cudaMemcpyAsync(dst, src, siz, cudaMemcpyHostToDevice, stream))
...
bool CUDAMiner::initEpoch() {
    ...
    try {
        ...
        for (unsigned i = 0; i < m_deviceDescriptor.cuStreamSize; ++i) {
            try {
                // harusnya ini pake cudaMallocManaged, hash nya jadi naik turun :(
            ...
            // Biar kuenceng bro...was...wes...wossss
            CUDA_CALL(cudaDeviceGetStreamPriorityRange(&priority_low, &priority_high));
            CUDA_CALL(cudaStreamCreateWithPriority(&m_streams[i], cudaStreamNonBlocking, priority_high));
        }
        ...
        // yahh karena ada etc jadi di split deh, karena cache size nya beda2 cuy...
        if(m_deviceDescriptor.cuAlgo == "etchash") {
            HostToDevice(light, m_epochContext.etclightCache, m_epochContext.lightSize, m_streams[0]);
        } else {
            HostToDevice(light, m_epochContext.lightCache, m_epochContext.lightSize, m_streams[0]);
        }
    ...
    try {
        while (!shouldStop()) {
            ...
            if (current.epoch != last.epoch) {
                setEpoch(current, m_deviceDescriptor.cuAlgo);
                if (g_seqDAG)
                    g_seqDAGMutex.lock();
                bool b = initEpoch();
                if (g_seqDAG)
...
void CUDAMiner::enumDevices(minerMap& _DevicesCollection) {
    ...
    for (int i = 0; i < numDevices; i++) {
        ...
        try {
            ...
            // biar dapeting total core dalam 1 GPU
            deviceDescriptor.cuCores = _ConvertSMVer2Cores(props.major, props.minor) * props.multiProcessorCount;
            deviceDescriptor.cuBlockSize = 128;
            deviceDescriptor.cuStreamSize = 2;
            // 0 = dynamic kernel, 6 = choose by core size, 1 - 5 pilih core sendiri
            deviceDescriptor.cuKernel = 0; 
            deviceDescriptor.cuMP = props.multiProcessorCount;
...
void CUDAMiner::search(uint8_t const* header, uint64_t target, uint64_t start_nonce, const dev::eth::WorkPackage& w) {
    ...
    while (streams_bsy) {
        ...
        for (uint32_t streamIdx = 0; streamIdx < m_deviceDescriptor.cuStreamSize;
            ...
            HostToDevice(buffer, zero3, sizeof(zero3), stream);

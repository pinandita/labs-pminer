...
namespace etchash
{
constexpr auto revision = ETCHASH_REVISION;
...
using epoch_context = etchash_epoch_context;
using epoch_context_full = etchash_epoch_context_full;

using result = etchash_result;
...
/// Alias for etchash_calculate_light_cache_num_items().
static constexpr auto calculate_light_cache_num_items = etchash_calculate_light_cache_num_items;

/// Alias for etchash_calculate_full_dataset_num_items().
static constexpr auto calculate_full_dataset_num_items = etchash_calculate_full_dataset_num_items;

/// Alias for etchash_calculate_epoch_seed().
static constexpr auto calculate_epoch_seed = etchash_calculate_epoch_seed;
...
using epoch_context_ptr = std::unique_ptr<epoch_context, decltype(&etchash_destroy_epoch_context)>;

using epoch_context_full_ptr =
    std::unique_ptr<epoch_context_full, decltype(&etchash_destroy_epoch_context_full)>;

/// Creates etchash epoch context.
///
/// This is a wrapper for etchash_create_epoch_number C function that returns
...
{
    return {etchash_create_epoch_context(epoch_number), etchash_destroy_epoch_context};
}

inline epoch_context_full_ptr create_epoch_context_full(int epoch_number) noexcept
{
    return {etchash_create_epoch_context_full(epoch_number), etchash_destroy_epoch_context_full};
}
...
{
    return etchash_hash(&context, &header_hash, nonce);
}
...
{
    return etchash_verify_final_hash(&header_hash, &mix_hash, nonce, &boundary);
}
...
{
    return etchash_verify(&context, &header_hash, &mix_hash, nonce, &boundary);
}
...
{
    return *etchash_get_global_epoch_context(epoch_number);
}

/// Get global shared epoch context with full dataset initialized.
inline const epoch_context_full& get_global_epoch_context_full(int epoch_number) noexcept
{
    return *etchash_get_global_epoch_context_full(epoch_number);
}
}  // namespace etchash

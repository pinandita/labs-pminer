...
/// @file
/// Contains declarations of internal etchash functions to allow them to be
/// unit-tested.

#pragma once

#include <etchash/ethash.hpp>
...
extern "C" struct etchash_epoch_context_full : etchash_epoch_context
{
    etchash_hash1024* full_dataset;

    constexpr etchash_epoch_context_full(int epoch, int light_num_items, const etchash_hash512* light,
                                        const uint32_t* l1,
                                        int dataset_num_items, etchash_hash1024* dataset) noexcept
        : etchash_epoch_context{epoch, light_num_items, light, l1, dataset_num_items}, full_dataset{dataset} {}
};

...
using namespace etchash;

extern "C" {

etchash_hash256 etchash_calculate_epoch_seed(int epoch_number) noexcept
{
    etchash_hash256 epoch_seed = {};
    for (int i = 0; i < epoch_number; ++i)
        epoch_seed = etchash_keccak256_32(epoch_seed.bytes);
    return epoch_seed;
}
...
int etchash_calculate_light_cache_num_items(int epoch_number) noexcept
{
    ...
    int num_items = etchash_find_largest_prime(num_items_upper_bound);
    return num_items;
}
...
epoch_context* etchash_create_epoch_context(int epoch_number) noexcept
{
    return generic::create_epoch_context(build_light_cache, epoch_number, false);
}

epoch_context_full* etchash_create_epoch_context_full(int epoch_number) noexcept
{
    return generic::create_epoch_context(build_light_cache, epoch_number, true);
}

void etchash_destroy_epoch_context_full(epoch_context_full* context) noexcept
{
    etchash_destroy_epoch_context(context);
}

void etchash_destroy_epoch_context(epoch_context* context) noexcept
{
    context->~epoch_context();
    std::free(context);
}

etchash_result etchash_hash(
...
bool etchash_verify_final_hash(const hash256* header_hash, const hash256* mix_hash, uint64_t nonce,
...
bool etchash_verify(const epoch_context* context, const hash256* header_hash,

...
static void on_algo(string algo) {
    string chk = boost::algorithm::to_lower_copy(algo);
    if(chk == "ethash") {
        algo = 1;
    } else if(chk == "etchash") {
        algo = 2;
    } else {
        throw boost::program_options::error("The --algo supporting value is 'ethash, etchash' ");   
    }    
}
...
// prepare for auto kernel each device
static void on_cu_kernel(unsigned k) {
    if((k >= 0) && (k < 9)) 
        return;
    throw boost::program_options::error("The --cu-kernel value is from 0 - 8");   
}
...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv) {
        ...
        cu.add_options()
            ...
            ("cu-kernel", value<unsigned>()->default_value(0)->notifier(on_cu_kernel),

                "Set the number of streams per GPU, valid values from 0 to 8. default: 0 (dynamic kernel)");
        ...
        m_FarmSettings.cuKernel = vm["cu-kernel"].as<unsigned>();
#endif

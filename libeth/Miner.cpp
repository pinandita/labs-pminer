...
void Miner::setEpoch(WorkPackage const& w, string algo="etchash") {
    if(algo == "etchash") {
        etchash::epoch_context ec = etchash::get_global_epoch_context(w.epoch);
        m_epochContext.epochNumber = w.epoch;
        m_epochContext.lightNumItems = ec.light_cache_num_items;
        m_epochContext.lightSize = etchash::get_light_cache_size(ec.light_cache_num_items);
        m_epochContext.dagNumItems = ec.full_dataset_num_items;
        m_epochContext.dagSize = etchash::get_full_dataset_size(ec.full_dataset_num_items);    
        m_epochContext.etclightCache = new etchash_hash512[m_epochContext.lightNumItems];
        memcpy(m_epochContext.etclightCache, ec.light_cache, m_epochContext.lightSize);
    } else {

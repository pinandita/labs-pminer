    ...
    if (!m_miners.size()) {
        for (auto it = m_DevicesCollection.begin(); it != m_DevicesCollection.end(); it++) {
            ...
            if (it->second.subscriptionType == DeviceSubscriptionTypeEnum::Cuda) {
                ...
                if (m_Settings.cuKernel)
                    it->second.cuKernel = m_Settings.cuKernel;
                if(m_Settings.cuAlgo != "")
                    it->second.cuAlgo = m_Settings.cuAlgo;
                //m_miners.push_back(shared_ptr<Miner>(new CUDAMiner(m_miners.size(), it->second)));
                m_miners.push_back(std::shared_ptr<Miner>((new CUMinerFactory())->GetCUMinerIntance(m_miners.size(), it->second)));
            }
#endif
#if ETH_ETHASHCL
...
void Farm::submitProofAsync(Solution const& _s) {
    Result r;
    if(m_Settings.cuAlgo == "ethash") {
        r = EthashAux::eval(_s.work.epoch, _s.work.header, _s.nonce);
    } else {
        r = EthashAux::etc_eval(_s.work.epoch, _s.work.header, _s.nonce);
    }

...
Result EthashAux::etc_eval(int epoch, h256 const& _headerHash, uint64_t _nonce) noexcept {
        auto headerHash = etchash::hash256_from_bytes(_headerHash.data());
        auto& context = etchash::get_global_epoch_context(epoch);
        auto result = etchash::hash(context, headerHash, _nonce);
        h256 mix{reinterpret_cast<byte*>(result.mix_hash.bytes), h256::ConstructFromPointer};
        h256 final{reinterpret_cast<byte*>(result.final_hash.bytes), h256::ConstructFromPointer};
        return {final, mix};
}
